...
while ciclo<numCiclo and erroQ>erroMin:
    ...
    ##################### FASE DE RETROPROPAGAÇÃO
        ...
        deltinhaw = (t[i] - y)*(1/(1+math.exp(-yin))) * (1 - (1/(1+math.exp(-yin))))
        deltabw=alfa*deltinhaw

        for j in range(neuroniosescondidos):
            deltaw[j]=alfa*deltinhaw*z[j]
            deltinhav[j]=deltinhaw*w[j]*(1/(1+math.exp(-zin[j])))*(1-(1/(1+math.exp(-zin[j]))))
...
#Passo5
    ...
    if(teste_saida[i]==1.0):
        if ((teste_saida[i]-y)<0.3):
